...
export function Network() {
  ...
  const { currentUser, isAuthenticated } = useAuth0();
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        ...
        // Check if response is an empty array or contains data
        if (Array.isArray(res.data)) {
          const usersWithFlag = res.data.map((user: any) => ({
            ...user,
  ...
  return (
      <div className="p-6 max-w-5xl mx-auto">
        <div className="flex items-center space-x-4 mb-6">
          <button
              className={`px-4 py-2 rounded-full ${activeTab === 'discover' ? 'bg-green-600 text-white' : 'bg-gray-200'}`}
              onClick={() => setActiveTab('discover')}
          >
            Discover
          </button>
          <button
              className={`px-4 py-2 rounded-full ${activeTab === 'followers' ? 'bg-green-600 text-white' : 'bg-gray-200'}`}
              onClick={() => setActiveTab('followers')}
          >
            Followers
          </button>
          <button
              className={`px-4 py-2 rounded-full ${activeTab === 'following' ? 'bg-green-600 text-white' : 'bg-gray-200'}`}
              onClick={() => setActiveTab('following')}
          >
            Following
          </button>
        ...
        {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {filteredUsers.map(user => (
                  <div key={user.id} className="bg-white shadow rounded-xl p-4 flex items-start space-x-4">
